...
The `webroot.xml` screen is the default root screen in the OOTB runtime directory (`runtime/base-component/webroot/screen/webroot.xml`) and has a good example of including templates for different render modes:
...
This is an example of a screen with subscreens so it has *render-mode* elements before and after the active subscreen to decorate (or wrap) what comes from the child. The middle block calls *sri.renderSubscreen()* (you can also use the *subscreens-active* widget, as `apps.xml` does). This shows text elements with a **location** to include a FreeMarker template and inline *text* in a CDATA block right under the *text* element.
