...
A FinancialAccount is a single-entry balance account like a bank account. There are various types of financial account defined with the FinancialAccountType entity with settings like **isRefundable**, **requirePinCode**, automatic replenishment settings, and others. OOTB types include Gift Certificate, Gift Card, Customer Credit, Service Credit, and Loan.
...
A transaction (FinancialAccountTrans) for a given **amount** may be a Deposit, Withdraw, or Adjustment (**finAccountTransTypeEnumId**). Transactions requiring approval or for other reasons may have a **statusId** of Created, Approved, or Cancelled. They will generally have a reason (**reasonEnumId**) such as Purchase, Initial Deposit, Replenishment, or Refund.
...
Depending on the direction and which Party is the internal organization there is a different set of statuses (**statusId**). For incoming invoices the statuses are Incoming, Received, Approved, Payment Sent, Billed Through, and Cancelled. For outgoing invoices the statuses are In-Process, Finalized, Sent, Acknowledged, Payment Received, Write Off, and Cancelled. There are also dedicated StatusFlow records for receivable and payable invoices, including transitions between them when both from and to parties are internal organizations.
...
Just like order items, invoice items may have a hierarchical structure using the **parentItemSeqId** field. This is used for things like tax items that are for a particular good or service item. An item may also point at an item on another invoice with **parentInvoiceId** and **parentInvoiceItemSeqId**.
...
![MantleDataModel-account.invoice](/docs/attachment/100462/MantleDataModel-account.invoice.svg)

## Account - Method (mantle.account.method)

A PaymentMethod is an instrument used for payment and each type has a separate entity with details including BankAccount, BitcoinWallet, CreditCard, GiftCard, and PayPalAccount. A PaymentMethod may be for a FinancialAccount and that is specified with the **finAccountId** field. Some payment method types such as cash, checks, and money orders are used directly on payments, orders, and so on with no PaymentMethod record because the Payment is not processed through a payment method.

A payment method is owned by a Party (**ownerPartyId**), has a **description**, and generally has a **postalContactMechId**, **telecomContactMechId**, and possibly an **emailContactMechId**.

A PaymentMethod is valid in a date range (**fromDate**, **thruDate**). Generally the **thruDate** field is null until the payment method is no longer used, or has been changed. PaymentMethod and related records are considered immutable, so when changed the original record has the **thruDate** set and a new record is created with the modified details. The new record points to the original with the **originalPaymentMethodId** field.
...
For GiftCard payment methods they are usually purchased from or issued by the organization and details about that are tracked with the GiftCardFulfillment entity.
...
Any time a payment gateway is used the details of the response should be stored with the PaymentGatewayResponse entity. They are generally associated with a Payment (**paymentId**) and have various fields for codes and results from the payment processor.

![MantleDataModel-account.method](/docs/attachment/100462/MantleDataModel-account.method.svg)
...
A Payment should always have a payment instrument (**paymentInstrumentEnumId**) such as cash, check, or credit card and if applicable for the instrument should also have a payment method (**paymentMethodId**).
...
A payment has various statuses (**statusId**) including Proposed, Promised, Authorized, Delivered, Confirmed Paid, Cancelled, Void, and Declined.
...
For fraud sensitive organizations and applications when processing online transactions it is important to associate the Payment with a Visit using the **visitId** field. This tracks the client IP address and other HTTP client and session information. When a fraudulent transaction is identified the evidence should be recorded in a PaymentFraudEvidence and this is usually used to change the trust level on the associated payment method (PaymentMethod.**trustLevelEnumId**) and contact mechs (ContactMech.**trustLevelEnumId**).
...
The accounting transaction (AcctgTrans) generated for automated posting of a payment will have one entry posted to the GL account (GlAccount) configured for the cash account for the payment instrument (unless overrideGlAccountId is populated, then that is used), and a balancing transaction entry posted to an accounts payable account for outgoing payments and an accounts receivable account for incoming payments.
...
![MantleDataModel-account.payment](/docs/attachment/100462/MantleDataModel-account.payment.svg)

## Ledger - Account (mantle.ledger.account)

General ledger accounts (GlAccount) make up the chart of accounts for an internal Organization. Each account has a class (**glAccountClassEnumId**) to determine if the account balance is added or subtracted to a transaction total and for reporting purposes (especially: Balance Sheet with Asset on one side and Liability and Equity on the other; and Income Statement with Revenue, Cost of Sales, Income and Expense accounts). Contra accounts are descendants of those classes with the Enumeration **optionIndicator** set rather than separate top-level contra classes. Here is the structure of the OOTB GL account classes (this can be changed with different Enumeration records of type GlAccountClass):
