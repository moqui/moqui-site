...
The default Request statuses (**statusId**) include Draft, Submitted, Reviewed, In Progress, Completed, and Cancelled. It also has a resolution (**requestResolutionEnumId**) that is by default Unresolved and default options include Fixed, Can't Reproduce, Won't Fix, Duplicate, Rejected, and Insufficient Information. Additional resolutions can be added as Enumeration records of type RequestResolution. If the result should be emailed, where to send it is specified with the **emailContactMechId** field.

A Request has a name (**requestName**) and **description**. Additional story text and other documents are associated with RequestContent (content type Story, among others). To help determine the order to work on requests and for general information it has **priority**, **requestDate**, and **responseRequiredDate** fields. A request may be associated with a Facility (**facilityId**) and ProductStore (**productStoreId**).
...
After requirements are created they can be summarized by Product and Facility then after a supplier is selected an order with the total quantity can be created and associated with the RequirementOrderItem entity.
