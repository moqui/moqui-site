...
Transfer shipments are used to transfer inventory from one facility to another. They use pack and inventory issuance from the origin
facility just like outgoing shipments as well as inventory receipt just like incoming shipments.

## Create, Pack and Ship a Transfer Shipment

- start on Find Shipment screen
- use *Create Transfer Shipment* dialog, specify from and to facilities
...
- if a shipping label is needed
    ...
    - use the *Get Labels* near the top of the screen or the *Get Label* button for each package to get labels
...
- use the 'Receive Product' to receive into inventory in destination Facility
